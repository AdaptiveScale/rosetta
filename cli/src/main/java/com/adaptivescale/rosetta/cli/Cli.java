...
import com.adaptivescale.rosetta.test.TestFactory;
import com.adaptivescale.rosetta.test.Tester;
...
class Cli implements Callable<Void> {
    ...
    @CommandLine.Command(name = "test", description = "Compare model again database, check changes", mixinStandardHelpOptions = true)
    private void test(@CommandLine.Option(names = {"-s", "--source"}) String sourceName) throws Exception {
        ...
        Tester<List<String>, Database, Database> tester = TestFactory.tester();

        List<String> changeList = tester.test(localDatabase, targetDatabase);
