...
#### query
The query command helps you query your databases using natural language commands, converting them into SQL SELECT statements. This command leverages the capabilities of OpenAI models to interpret user queries and generate corresponding SQL queries. To utilize this command effectively, users must provide their OpenAI API Key and specify the OpenAI model to be used, output will be written in a CSV file.   
...
--no-limit Response Row limit (Optional) | Specifies that there should be no limit on the number of rows in the generated CSV file.
